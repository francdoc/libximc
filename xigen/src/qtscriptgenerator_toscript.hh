#ifndef QTSCRIPTGENERATOR_TOSCRIPT_HH
#define QTSCRIPTGENERATOR_TOSCRIPT_HH
...
namespace xigen
{
	...
	class QtscriptGenerator_toscript : protected DefaultVisitor, public Noncopyable, public BaseGenerator
	{
		protected:
			...
			virtual void visitCommandImpl (Command& command)
			{
				...
				if (command.withAnyFields() && !command.is("inline"))
				{
					pushState( command );
					stream()
						<< "Q_DECLARE_METATYPE(" << command.structName() << "_t);" << "\n"
						<< "static QScriptValue toScriptValue(QScriptEngine *engine, const " << command.structName() << "_t" << " &s) {" << "\n"
		...
		public:
			...
			explicit QtscriptGenerator_toscript ()
				:	m_current(NULL)
			{
			}
