"""
 Python script that interfaces with a library called "libximc" using ctypes. 
 "libximc" is a library that allows communication with motor controllers and stages from the "XILAB" series developed by STANDA Ltd., which are used to control motors and stages for various motion control applications.
 The code contains various classes and structures that define settings and flags used to configure and control the motors and stages. It provides functions to enumerate devices, get device information, and set various motor and stage parameters.
 """
...
def ximc_shared_lib():
    if platform.system() == "Linux":
        ...
        if sys.version_info[0] == 3 and sys.version_info[0] >= 8:
            return WinDLL("C:\Program Files\XILab\libximc.dll", winmode=RTLD_GLOBAL)
        else:
            return WinDLL("C:\Program Files\XILab\libximc.dll")
    else:
        return None
...
class Result:
    ...
    NotImplemented = -2
    ValueError = -3
    NoDevice = None # this error happens if USB cable is disconnected or device is not powered
...
class EngineFlags:
	ENGINE_REVERSE           = 0x01 # THIS IS KEY
...
class FeedbackType:
	FEEDBACK_ENCODER             = 0x01 # THIS IS KEY
	FEEDBACK_EMF                 = 0x04
	FEEDBACK_NONE                = 0x05
...
class FeedbackFlags:
	...
	FEEDBACK_ENC_TYPE_SINGLE_ENDED    = 0x41 # THIS IS KEY. 0x40 (original) -> 0x41 seems to be the correct hex.
...
class EncoderSettingsFlags:
	...
	ENCSET_PUSHPULL_OUTPUT                 = 0x004 # THIS IS KEY
...
class feedback_settings_t(Structure): # necessary for moving Front XY with encoders
	_fields_ = [
		("IPS", c_uint),
		("FeedbackType", c_uint),
		("FeedbackFlags", c_uint),
		("CountsPerTurn", c_uint), # THIS IS KEY -> CountsPerTurn = 4000
	]
...
class engine_settings_t(Structure): #  necessary for moving Standa HSI XY
	_fields_ = [
		("NomVoltage", c_uint),
		("NomCurrent", c_uint),
		("NomSpeed", c_uint),
		("uNomSpeed", c_uint),
		("EngineFlags", c_uint), # EngineFlags need -> ENGINE_ANTIPLAY -> for backlash compensation.
		("Antiplay", c_int), # Antiplay -> Number of pulses or steps for backlash (play) compensation procedure. Used if ENGINE_ANTIPLAY flag is set.
		("MicrostepMode", c_uint),
		("StepsPerRev", c_uint),
	]
...
class edges_settings_calb_t(Structure): # for limit switches
	_fields_ = [
		("BorderFlags", c_uint),
		("EnderFlags", c_uint),
		("LeftBorder", c_float),
...
class pid_settings_t(Structure): # https://doc.xisupport.com/en/8smc5-usb/8SMC5-USB.pdf -> pg 64 - 4.2.8.3 PID regulator tuning recommendations -> 4.2.8.1 Algorithm description -> set_pid_settings()
	_fields_ = [
		("KpU", c_uint),
		("KiU", c_uint),
		("KdU", c_uint),
...
class get_position_t(Structure):
	_fields_ = [
		("Position", c_int), # Is only used with stepper motor.
		("uPosition", c_int), # Is only used with stepper motor.
		("EncPosition", c_longlong), # Current encoder position.
	]
...
class gear_settings_calb_t(Structure):
	_fields_ = [
		("ReductionIn", c_float),
		("ReductionOut", c_float),
		("RatedInputTorque", c_float),
